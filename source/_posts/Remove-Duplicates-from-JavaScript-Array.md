...
最基本的，我们迭代整个数组，检查其中每一个元素在数组中第一次出现的位置与该元素实际的位置是否相等，很显然如果不等则为重复元素。
使用 `filter` 的回调函数中的第三个参数 `self` ，可以避免数组变量的闭包问题。
...
同理，由于 `sort` 方法无法直接对物件进行排序，因此这种方法对物件无效。另外存在一个副作用————原始数组已被改变。假如输入数组已经是有序的，则可以使用这种方法（同时去掉上述代码中的 `sort`）。

# Unique by...

有时候希望能基于一些条件而不仅仅是相等将数组唯一化，例如过滤掉不同的物件，但是分享其中一些性质。这可以用传递回调函数优美地解决。`key` 回调函数会应用于每一个元素，只要元素等于 "keys" 则会被移除。由于 `key` 预期会返回一个原始类型，因此哈希表在这里可以正常工作：
...
其时间复杂度是二次方，但是有些额外的好处，例如封装了原生的 `indexOf` 方法、可以通过键进行唯一化（官方说法叫 `iteratee`）、对于有序数组的优化。

如果使用 jQuery 并且喜欢以美刀符号开头，大概长得这样：
...
JavaScript 中函数呼叫的成本很昂贵，因此上述方法即使简洁明了，但是并不高效。为了最高效能，将 `filter` 全部替换成回圈以避免多余的函数呼叫：
...
这段丑陋的代码和第三段代码做了相同的事情，但是快了一个数量级。
